...
import { createUserWithoutOrganization, inviteToOrganization } from './keycloak.ts';
 ...
 */
export async function latestMail(address: string, path?: string): Promise<Brev> {
  for (let attempt = 0; attempt < 60; attempt++) {
    ...
    for (const candidate of list.messages) {
      if (candidate.To[0]?.Address.toLowerCase() !== address.toLowerCase()) continue;

      const brev = (await (
        await fetch(`${MAILPIT}/api/v1/message/${candidate.ID}`)
      ).json()) as Omit<Brev, 'ID'>;

      if (!path || avkoda(brev.Text).includes(path)) return { ID: candidate.ID, ...brev };
    }
    await new Promise((resolve) => setTimeout(resolve, 250));
  }
  throw new Error(`Inget mail till ${address}${path ? ` med ${path}` : ''} inom rimlig tid`);
}
...
async function clickMailLink(page: Page, who: Person, path: string): Promise<void> {
  ...
  const iBrevet = new RegExp(`https?://\\S*${path}\\S*`).exec(avkoda(brevet.Text))?.[0];
...
export async function acceptInvitation(page: Page, who: Person): Promise<void> {
  ...
  await inviteToOrganization(page, who.email, who.organization, who.displayName);
...
export async function inviteAndOnboard(page: Page, who: Person): Promise<void> {
   ...
   */
  await page.waitForURL(/\/auth\/realms\/fastgig\/account/, { timeout: 15_000 }).catch(() => {});
...
export async function unaffiliatedAccount(page: Page, who: Person): Promise<void> {
  await ensureSignedOut(page);
  await createUserWithoutOrganization(page, who.email, who.password, who.displayName);
}
...
export async function publishSpec(
  ...
  const token = await page.evaluate(() => {
    ...
    return key ? (JSON.parse(sessionStorage.getItem(key)!) as { access_token: string }).access_token : null;
  });
