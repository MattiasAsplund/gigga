...
test("hela flödet från förfrågan till signerat avtal", async ({ page }) => {
	...
	await test.step("4. Kim publicerar en förfrågan", async () => {
		...
		await expect(page.getByTestId("request-title")).toHaveText(
			"Bygg en Fortnox-integration",
		);
	...
	await test.step("8. Kim läser anbudet och hämtar dokumenten som ZIP", async () => {
		...
		await expect(page.getByTestId("bid-seller")).toContainText(
			robin.displayName,
		);
	...
	await test.step("9. Båda signerar och avtalet blir bindande", async () => {
		...
		await expect(page.getByTestId("signature-buyer")).toHaveAttribute(
			"data-signed",
			"true",
		);
		await expect(page.getByTestId("signature-seller")).toHaveAttribute(
			"data-signed",
			"false",
		);
...
test("säljaren kan ändra och dra tillbaka sitt anbud", async ({ page }) => {
	...
	await page.getByTestId("submit-bid").click();
	const ändringsBidId = (await page
		.getByTestId("bid")
		.first()
...
test("en bekräftelselänk som inte gäller ger besked, inte ett rått felsvar", async ({
	 ...
	 */
	await page.goto(
		"/auth/realms/gigga/login-actions/action-token?key=inte-en-riktig-nyckel",
	);
