...
interface FastgigClaims extends JWTPayload {
  email?: string;
 ...
 */
function organizationOf(claims: FastgigClaims): string {
  const raw = claims.organization;
  const aliases = (Array.isArray(raw) ? raw : raw ? [raw] : []).filter((a) => a.length > 0);
...
export async function registerAuth(app: FastifyInstance): Promise<void> {
  ...
  app.decorate('requireAuth', async (req: FastifyRequest, _reply: FastifyReply) => {
    ...
    if (!token) throw unauthorized('Token saknas, är utgången eller ogiltig.');

    let claims: FastgigClaims;
    try {
      const verified = await jwtVerify<FastgigClaims>(token, app.keys.resolve, {
