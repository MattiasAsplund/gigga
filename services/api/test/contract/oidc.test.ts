...
const me = (token: string) =>
  t.app.inject({ method: 'GET', url: '/api/v1/me', headers: { authorization: `Bearer ${token}` } });

// ---------------------------------------------------------------- O.1

test('O.1 en giltig token från realmet släpps in', async () => {
  const token = await signToken({ subject: crypto.randomUUID(), email: 'o1@example.test' });
...
test('O.2 saknad, tom och trasig Authorization ger 401', async () => {
  ...
  for (const header of ['', 'Bearer ', 'Bearer inte.en.token', 'Basic abc123']) {
...
test('O.3 en manipulerad signatur ger 401', async () => {
  const token = await signToken({ subject: crypto.randomUUID(), email: 'o3@example.test' });
...
test('O.4 fel issuer ger 401 — en annan realm duger inte', async () => {
  const token = await signToken({
    ...
    issuer: 'http://angriparen.test/auth/realms/fastgig',
  });
...
test('O.10 speglingen skapas en gång och står still över flera anrop', async () => {
  ...
  const token = await signToken({ subject, email: 'o10@example.test', organizations: ['o10-ab'] });

  const first = (await me(token)).json<{ id: string; organization: { id: string } }>();
  const second = (await me(token)).json<{ id: string; organization: { id: string } }>();
...
test('O.11 ändrad adress och namn i Keycloak slår igenom i speglingen', async () => {
  ...
  const before = (
    await me(await signToken({ subject, email: 'gammal@example.test', displayName: 'Gammalt' }))
  ).json<{ id: string }>();

  const after = (
    await me(await signToken({ subject, email: 'ny@example.test', displayName: 'Nytt' }))
...
test('O.12 två konton i samma organisation delar organisationsrad', async () => {
  const [a, b] = await Promise.all([
    me(await signToken({ subject: crypto.randomUUID(), email: 'o12a@example.test', organizations: ['o12-ab'] })),
    me(await signToken({ subject: crypto.randomUUID(), email: 'o12b@example.test', organizations: ['o12-ab'] })),
  ]);
...
test('O.13 /me lämnar ut det lokala id:t, inte Keycloaks subjekt', async () => {
  ...
  expect(TEST_ISSUER).toContain('/realms/fastgig');
});
