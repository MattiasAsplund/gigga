...
async function createRequest(): Promise<string> {
	...
	if (res.statusCode !== 201)
		throw new Error(`Kunde inte skapa förfrågan: ${res.body}`);
...
async function requestWithBid(): Promise<{ requestId: string; bidId: string }> {
	...
	if (res.statusCode !== 201)
		throw new Error(`Kunde inte lämna anbud: ${res.body}`);
...
test("Ä.2 fälten går att ändra var för sig", async () => {
	...
	const onlyPlan = await changeBid(seller, bidId, {
		plan: "Bara planen är ny.",
	});
	expect(onlyPlan.statusCode).toBe(200);
	expect(onlyPlan.json<BidBody>().plan).toBe("Bara planen är ny.");
	// Ersättningen står kvar orörd.
	expect(onlyPlan.json<BidBody>().compensation).toEqual(
		fixedBid.compensation as never,
	);
...
test("Ä.4 en annan säljare får inte ändra anbudet", async () => {
	...
	const res = await changeBid(rival, bidId, {
		plan: "Jag skriver om ditt anbud.",
	});

	expect(res.statusCode).toBe(403);
	expect(res.json<Problem>().type).toBe(
		"https://gigga.dev/problems/not-bid-owner",
	);
});
...
test("Ä.4b köparen får inte heller ändra anbudet", async () => {
	...
	const res = await changeBid(buyer, bidId, {
		plan: "Jag justerar ditt anbud åt dig.",
	});
...
test("Ä.5 okänt anbud ger 404", async () => {
	...
	expect(res.json<Problem>().type).toBe(
		"https://gigga.dev/problems/bid-not-found",
	);
});
...
test("Ä.7 ändring efter deadline ger 422", async () => {
  ...
  `;
	...
	expect(res.statusCode).toBe(422);
	expect(res.json<Problem>().errors?.map((e) => e.path)).toContain(
		"deadlineAt",
	);
});
...
test("Ä.9 ändring när avtalet finns ger 409", async () => {
	const { bidId } = await requestWithBid();
	const signed = await buyer.post(
		`/api/v1/bids/${bidId}/contract/signatures`,
		{},
	);
	expect(signed.statusCode).toBe(200);

	const res = await changeBid(seller, bidId, {
		plan: "Villkoren är redan frysta.",
	});

	expect(res.statusCode).toBe(409);
	expect(res.json<Problem>().type).toBe(
		"https://gigga.dev/problems/contract-exists",
	);
});
