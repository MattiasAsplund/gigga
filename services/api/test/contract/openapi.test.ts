...
const EXPECTED_OPERATIONS: Record<string, [string, string]> = {
	...
	downloadContractDocument: ["get", "/api/v1/contracts/{contractId}/document"],
	...
	revokeRequestPermission: [
		"delete",
		"/api/v1/requests/{requestId}/permissions/{userId}",
	],
	...
	downloadAttachmentArchive: [
		"get",
		"/api/v1/bids/{bidId}/attachments/archive",
	],
	renameAttachment: [
		"patch",
		"/api/v1/bids/{bidId}/attachments/{attachmentId}",
	],
	deleteAttachment: [
		"delete",
		"/api/v1/bids/{bidId}/attachments/{attachmentId}",
	...
	changeSpecCriterion: [
		"patch",
		"/api/v1/requests/{requestId}/spec/criteria/{criterionId}",
	],
	removeSpecCriterion: [
		"delete",
		"/api/v1/requests/{requestId}/spec/criteria/{criterionId}",
...
): { id: string; op: Operation; where: string }[] {
...
test("X.1 alla API:erna finns på rätt metod och väg", () => {
	for (const [operationId, [method, path]] of Object.entries(
		EXPECTED_OPERATIONS,
	)) {
...
test("X.2 varje operation har unikt operationId, tags och summary", () => {
	...
	for (const { id, op, where } of operations(doc)) {
		...
		expect(
			seen.has(id),
			`${where} har ett operationId som redan används: ${id}`,
		).toBe(false);
		seen.add(id);
...
test("X.2 skyddade operationer deklarerar bearerAuth, öppna gör det inte", () => {
	...
	for (const { id, op, where } of operations(doc)) {
		if (PROTECTED.has(id)) {
			expect(op.security, `${where} saknar security`).toEqual([
				{ bearerAuth: [] },
			]);
		} else {
...
test("X.2 varje skyddad operation dokumenterar 401 och 403", () => {
	...
	for (const { id, op, where } of operations(doc)) {
		...
		expect(
			op.responses?.["401"],
			`${where} dokumenterar inte 401`,
		).toBeDefined();
		expect(
			op.responses?.["403"],
			`${where} dokumenterar inte 403`,
		).toBeDefined();
	}
});
...
test("X.4 okänd väg ger 404 i Problem Details-format", async () => {
	const res = await ctx.app.inject({
		method: "GET",
		url: "/api/v1/finns-inte",
	});

	expect(res.statusCode).toBe(404);
	expect(res.headers["content-type"]).toContain("application/problem+json");
	expect(
		res.json<{ type: string; title: string; status: number }>(),
	).toMatchObject({
		type: "https://gigga.dev/problems/not-found",
		status: 404,
	});
});
...
test("X.4 fel metod på en känd väg ger 404 i samma format", async () => {
	...
	expect(res.json<{ type: string }>().type).toBe(
		"https://gigga.dev/problems/not-found",
	);
});
