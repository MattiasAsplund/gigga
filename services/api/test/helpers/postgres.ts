 ...
 *
 *   podman rm -f gigga-test-pg   # riv den för hand vid behov
...
const CONTAINER = "gigga-test-pg";
const TEMPLATE_DB = "gigga_template";
...
async function isRunning(): Promise<boolean> {
	const res = await $`podman inspect -f {{.State.Running}} ${CONTAINER}`
		.nothrow()
		.quiet();
...
async function waitForReady(): Promise<void> {
	...
	while (Date.now() < deadline) {
		const probe = await $`podman exec ${CONTAINER} pg_isready -U test -q`
			.nothrow()
			.quiet();
	...
	throw new Error(
		`Postgres blev inte redo inom ${READY_TIMEOUT_MS} ms.\n${logs}`,
	);
}
